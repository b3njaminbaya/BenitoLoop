...
short-lived signed URLs. A donation-category suggestion is generated by an
image-classification model (TensorFlow.js + MobileNet) that runs entirely
in the donor's browser — no photo is ever sent to a third-party AI service,
and no API key or paid service is required.
...
callback handling and for sending transactional email. On-device machine
learning for donation photo triage runs via TensorFlow.js and a MobileNet
model, loaded lazily so it never impacts the initial page load.
...
src/
  ...
  lib/            Domain logic and Supabase client calls, one module per
                  ...
                  donation photos, the M-Pesa client, the on-device
                  classifier, etc.)
